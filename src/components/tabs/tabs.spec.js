describe('<md-tabs>', function() {
  ...
  describe('activating tabs', function() {
    ...
    it('should focus tab on arrow if tab is enabled', inject(function($document, $mdConstant, $timeout) {
      ...
      triggerKeydown(tabs.find('md-dummy-tab').eq(0), $mdConstant.KEY_CODE.LEFT_ARROW);
      expect(tabItems.eq(0)).toBeActiveTab();

      // Tab 0 should still be active, but tab 2 focused (skip tab 1 it's disabled)
      triggerKeydown(tabs.find('md-dummy-tab').eq(0), $mdConstant.KEY_CODE.RIGHT_ARROW);
      expect(tabItems.eq(0)).toBeActiveTab();

      triggerKeydown(tabs.find('md-dummy-tab').eq(2), $mdConstant.KEY_CODE.ENTER);
      ...
      triggerKeydown(tabs.find('md-dummy-tab').eq(2), $mdConstant.KEY_CODE.LEFT_ARROW);
      triggerKeydown(tabs.find('md-dummy-tab').eq(0), $mdConstant.KEY_CODE.ENTER);
      expect(tabItems.eq(0)).toBeActiveTab();
    ...
    it('should select tab on space or enter', inject(function($mdConstant) {
      ...
      triggerKeydown(tabs.find('md-dummy-tab').eq(1), $mdConstant.KEY_CODE.ENTER);
      expect(tabItems.eq(1)).toBeActiveTab();

      triggerKeydown(tabs.find('md-dummy-tab').eq(0), $mdConstant.KEY_CODE.SPACE);
      expect(tabItems.eq(0)).toBeActiveTab();
    }));
